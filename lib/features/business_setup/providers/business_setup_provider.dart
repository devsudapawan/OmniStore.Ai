...
class BusinessSetupNotifier extends StateNotifier<BusinessSetupState> {
  ...
  Future<void> completeBusinessSetup() async {
    ...
    try {
      print('🚀 [Setup] Starting business setup...');
      print('🔍 [Setup] State: ${state.selectedStoreType}, ${state.businessName}, ${state.creditEnabled}');

      // Step 1
      print('📌 [Setup] Step 1: createBusiness...');
      final businessId = await _repository.createBusiness(state);
      print('✅ [Setup] Step 1 done. businessId=$businessId');
      _markStep(1);

      // Step 2
      print('📌 [Setup] Step 2: createPaymentMethods...');
      await _repository.createPaymentMethods(state);
      print('✅ [Setup] Step 2 done.');
      _markStep(2);

      // Step 3
      print('📌 [Setup] Step 3: createCategories...');
      await _repository.createCategories(state);
      print('✅ [Setup] Step 3 done.');
      _markStep(3);

      // Step 4
      print('📌 [Setup] Step 4: saveSettings...');
      await _repository.saveSettings(state);
      print('✅ [Setup] Step 4 done.');
      _markStep(4);

      print('🎉 [Setup] All steps complete!');
      state = state.copyWith(setupStatus: SetupStatus.success);

    } catch (e, stackTrace) {
      // Full error + stack trace printed to console
      print('❌ [Setup] FAILED at step ${state.completedSteps + 1}');
      print('❌ [Setup] Error: $e');
      print('❌ [Setup] StackTrace: $stackTrace');

      state = state.copyWith(
        setupStatus: SetupStatus.error,
        errorMessage: 'Step ${state.completedSteps + 1} failed:\n${e.toString()}',
      );
    }
  }
}
